...
### 任务群聊规则
- **建群由代码自动完成**：`task-manager.py add` 默认建群，`--no-chat` 跳过
- **不需要建群的任务**：定期检查、简单查询、快速操作 → 用 `--no-chat`
- 子任务用 `lark-send-message.sh` 在群里发重要进度（不要每步都发）
- **complete/fail 自动发结果**到任务群 + 源 chat（代码保证）
- **群不立即解散**：Carl 要看任务群的完整记录
- **24h 后自动清理**：`task-health-check.py` 解散完成超过 24h 的老群
...
| **项目进行中提新需求/担忧** | **replan 把新需求加入计划，绝不在主 session 动手调查** |
