...
def make_hourly_row(date_str, hour_str, tenant, usage_by_model, quota_data):
    ...
    if quota_data:
        for model_id in TRACKED_MODELS[:2]:  # 只显示前2个模型的配额
            ...
            if isinstance(remaining, (int, float)):
                row.append(format_number(remaining))
            else:
    ...
    for model_id in TRACKED_MODELS:
        ...
        row.append(format_number(model_usage.get("total", 0)))
...
def make_daily_row(day_data):
    """构造每日汇总行（每个租户独立模型用量，带千分位格式）"""
    # day_data 格式: {
    #   "date": "2026-02-12",
    #   "keys": [{"name": "Jose", "input": N, "output": N, "requests": N, "by_model": {...}}],
    #   "by_tenant": {"Jose": {"claude-opus": {"total": N}}, ...}
    # }
    date_str = day_data.get("date", "")
    
    rows = []
    # 优先使用 by_tenant 结构（新API）
    by_tenant = day_data.get("by_tenant", {})
    ...
    for key_info in day_data.get("keys", []):
        ...
        # 从 by_tenant 获取该租户的模型用量（带千分位）
        tenant_models = by_tenant.get(tenant, {})
        for model_id in TRACKED_MODELS:
            model_data = tenant_models.get(model_id, {})
            row.append(format_number(model_data.get("total", 0)))
